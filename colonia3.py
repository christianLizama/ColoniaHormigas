    ...
    for iteracion in range(iteraciones):
        ...
        for hormiga in range(num_hormigas):
            ...
            while ciudades_no_visitadas:
                ...
                # Verifica si hay probabilidades válidas
                probabilidades_validas = probabilidades[probabilidades > 0]

                if not probabilidades_validas.any():
                    break  # No hay ciudades válidas para moverse, termina el ciclo

                # Normalizar las probabilidades para que sumen 1
                probabilidades[probabilidades > 0] /= probabilidades_validas.sum()

                # Elegir la siguiente ciudad según las probabilidades
                siguiente_ciudad = np.random.choice(np.where(probabilidades > 0)[0], p=probabilidades[probabilidades > 0])
